...
func TestAggregatorSample(t *testing.T) {
	...
	assert.Contains(t, a.gauges, hashString32("gaugeTesttag1tag2"))

	a.count("countTest", 21, tags, 1)
	assert.Len(t, a.counts, 1)
	assert.Contains(t, a.counts, hashString32("countTesttag1tag2"))

	a.set("setTest", "value1", tags, 1)
	assert.Len(t, a.sets, 1)
	assert.Contains(t, a.sets, hashString32("setTesttag1tag2"))
